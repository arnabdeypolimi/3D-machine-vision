...
robot_pose = np.eye(4)
for i in range(len(dataset_handler)-1):

    image1 , depth1 = dataset_handler[i]
    image2 , depth2 = dataset_handler[i+1]

    rmat, tvec = get_pose(image1, image2, depth1, dataset_handler.k, display=True)
